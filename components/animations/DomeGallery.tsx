...
const photoImages = photographyProjects.flatMap(project => 
  project.media.filter(m => m.type === 'image').map(m => m.src)
);

const designImages = designProjects.flatMap(project => 
  (project.gallery || []).filter((m: any) => m.type === 'image').map((m: any) => m.src)
);

const allImages = [...photoImages, ...designImages];
const DEFAULT_IMAGES = shuffleArray(allImages);
...
export default function DomeGallery({
  ...
  maxRadius = Infinity,
  padFactor = 0.25,
  overlayBlurColor = '#080808', // Matched bg-bg-primary
  ...
  const [radiusState, setRadiusState] = useState(minRadius);
  const [viewerPadState, setViewerPadState] = useState(0);
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(entries => {
      ...
      setRadiusState(lockedRadiusRef.current);
      setViewerPadState(viewerPad);
  ...
  const autoRotateRAF = useRef<any>(null);
  useEffect(() => {
    const autoRotateSpeed = 0.05; // Speed of horizontal rotation
    const rotate = () => {
      if (isVisibleRef.current && !draggingRef.current && !openingRef.current && !inertiaRAF.current && !scrollLockedRef.current) {
  ...
  if (!isMounted) {
    return (
      <div className="sphere-root" style={{ height: '100%', width: '100%', backgroundColor: overlayBlurColor }} />
    );
  }
  ...
  return (
    <div
      ...
      style={{
        ['--radius' as any]: `${radiusState}px`,
        ['--viewer-pad' as any]: `${viewerPadState}px`,
